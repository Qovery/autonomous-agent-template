#!/usr/bin/env bash
# ────────────────────────────────────────────────────────────────────────────────
# Qovery Autonomous Agent — Main Run Script (Jira)
#
...
#   5. Comment the PR link on the Jira issue
...
# ── Load helpers ─────────────────────────────────────────────────────────────
SHARED_DIR=/usr/local/lib/agent
PROVIDER_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck disable=SC1091
source "$PROVIDER_DIR/jira.sh"
# shellcheck disable=SC1091
source "$SHARED_DIR/git-pr.sh"
# shellcheck disable=SC1091
source "$SHARED_DIR/callback.sh"
...
log "Fetching Jira issue ${JIRA_ISSUE_KEY}..."
TASK_FILE="/tmp/task.md"

if ! fetch_jira_issue "$JIRA_ISSUE_KEY" "$TASK_FILE"; then
  log_error "Failed to fetch Jira issue"
  post_callback "failed" "" "Failed to fetch Jira issue"
  exit 1
fi

ISSUE_TITLE=$(head -1 "$TASK_FILE" | sed 's/^# //')
log "Issue: ${JIRA_ISSUE_KEY} — ${ISSUE_TITLE}"
...
DEFAULT_SYSTEM_PROMPT="$SHARED_DIR/system-prompt.md"
...
BRANCH="agent/${JIRA_ISSUE_KEY}-${BRANCH_SLUG}"
...
case "$RDE_AUTONOMOUS_AGENT" in
  claude)
    ...
    # to stdout (container logs) and BFF /progress (agent session).
  ...
  codex)
    # ── Codex SDK execution ────────────────────────────────────────────────
    # Uses @openai/codex-sdk for programmatic streaming (same pattern as Claude).
    timeout "${RDE_RUN_TIMEOUT_MIN}m" runuser -u "$AGENT_USER" -- \
      env TASK_FILE="$TASK_FILE" PROGRESS_URL="$PROGRESS_URL" WORK_DIR="$WORK_DIR" \
      NODE_PATH="/usr/lib/node_modules" \
      node /usr/local/lib/agent/codex-runner.js 2>&1 | tee -a "$AGENT_LOG" || AGENT_EXIT=${PIPESTATUS[0]}
    AGENT_TAIL=$(tail -10 "$AGENT_LOG" 2>/dev/null | head -c 500 || true)
    ;;
  gemini)
    timeout "${RDE_RUN_TIMEOUT_MIN}m" runuser -u "$AGENT_USER" -- \
...
PR_BODY_INTRO="Automated fix by the Qovery autonomous agent for ${JIRA_ISSUE_KEY}."
...
for i in $(seq 1 "$REPO_COUNT"); do
  ...
  # dumped full of Jira context in the previous version of this script.
...
# The BFF callback handler posts issue comments, emits agent activities,
